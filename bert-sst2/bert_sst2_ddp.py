...
for epoch in range(1, num_epoch + 1):
    ...
    for inputs, targets in tqdm(train_dataloader,
        ...
        input_attn_mask = input_attn_mask.cuda(local_rank)
        input_tyi = input_tyi.cuda(local_rank)
    ...
    for inputs, targets in tqdm(test_dataloader, desc=f"Testing"):
        with torch.no_grad():
            input_ids, input_tyi, input_attn_mask = inputs[
    ...
    if epoch % check_step == 0 and local_rank == 0:
        # 保存模型
        checkpoints_dirname = "bert_sst2_ddp_" + timestamp
